  ...
  useEffect(() => {
    const fetchDashboardData = async () => {
      try {
        ...
        // This prevents one failed API (such as /services requiring
        // authentication) from hiding the other working data.
        ...
        if (
          ...
          setNotices(
            noticesRes.value.data.notices.slice(0, 3)
          );
        }
        ...
        if (
          ...
          assignmentsRes.value.data.success
        ) {
          setAssignments(
            assignmentsRes.value.data.assignments.slice(0, 3)
          );
        }

        // Placement drives
        if (
          drivesRes.status === 'fulfilled' &&
          drivesRes.value.data.success
        ) {
          setDrives(
            drivesRes.value.data.drives.slice(0, 3)
          );
        }

        // Services
        // This may fail if the user is not authenticated,
        // but that will no longer affect the other sections.
        if (
          servicesRes.status === 'fulfilled' &&
          servicesRes.value.data.success
        ) {
          setServices(
            servicesRes.value.data.requests.slice(0, 3)
          );
        }

        // Helpful console messages for debugging
        if (noticesRes.status === 'rejected') {
          console.warn('Failed to load notices:', noticesRes.reason);
        }
        ...
        if (servicesRes.status === 'rejected') {
          console.warn(
            'Services unavailable or authentication required:',
            servicesRes.reason
          );
        }
      } catch (err) {
  ...
  return (
    <div className="page-container">
      ...
      <div style={styles.banner}>
        <div>
          <div style={styles.bannerBadge}>
            ...
            <span>CampusOne Companion Hub</span>
          </div>
          ...
          <p style={styles.bannerText}>
            Here is your daily academic overview for{' '}
            <strong>
      ...
      <div style={styles.gridContainer}>
        ...
        {/* Study Focus Timer & Services Tracker Widget */}
        <div
          style={{
            display: 'flex',
